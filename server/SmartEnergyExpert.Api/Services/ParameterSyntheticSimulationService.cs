...
/// <summary>
/// Heuristic forward model for DSS demos—not a calibrated ocean-acoustic FEM.
/// Outputs tabular SPL samples compatible with CSV field imports and pairwise comparison.
/// </summary>
public sealed class ParameterSyntheticSimulationService : IParameterSyntheticSimulationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var minute = 0; minute < duration; minute++)
        {
            ...
            foreach (var bandHz in bands)
            {
                ...
                var baseLevel =
                    -68m // nominal receive level pedestal (relative / demo)
                    - bottomLossDb
                    - frequencyLossDb
                    + depthShelf
                    + thermalHue
    ...
    {
        // More negative NoiseLevelDb ⇒ calmer ⇒ smaller scatter.
    ...
    private static decimal BottomAttenuationDb(string normalizedBottom) =>
        normalizedBottom switch
        {
            ...
            "silt" => 6.05m,
            "soft_bottom" => 7.35m,
            _ => 2.95m // mixed / unknown seabed heuristic
        };
    ...
    {
        var kHzScale = decimal.Max(bandHz / 1000m, 0.18m); // heuristic monotonic attenuation versus centre frequency (kHz)
    ...
    /// <summary>
    /// Lightweight empirical SSP proxy (degrees C, PSU, metres).
    /// </summary>
